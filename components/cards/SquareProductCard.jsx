            ...
            <div key={data.id} >
                <div className='w-[270px] group flex flex-col gap-4 shadow-md transition-all'>
                    <div className='p-2 overflow-hidden bg-white relative'>
                        ...
                        <span className='text-base bg-secondary text-white font-third flex items-center justify-end absolute right-2 bottom-0 py-1 px-3 z-10 rounded-lg'>{data.price}<FaTurkishLiraSign className='text-xs' />
                        </span>
                        {
                            data.id === 1 ?
                            <span className='absolute top-0 right-0 p-2 bg-secondary text-white text-xs'>%10 İndirim</span>
                            :
                            data.id === 2 ?
                    ...
                    <div className='w-full flex gap-2 border-t-[1px] min-h-[77px] py-3 border-solid  border-slate-200 items-center justify-between px-4 pb-4'>
                        ...
                        <ButtonType url={`/urunler/${data.id}`} addClass="!py-1 !px-3 !text-base h-max" title={data.title}>İncele</ButtonType>
                    </div>
                </div>
            </div>
        </div>
