...
const CircleProductCard = ({ item }) => {
    return (
        <div>
            <div key={item.id} >
                <Link href={`/urunler/${item.id}`} title={item.title} className='w-[270px] group flex flex-col gap-4 transition-all'>
                    ...
                    <div className='w-full text-center flex flex-col gap-2 place-content-center'>
                        <h3 className='font-primary text-primary min-h-[48px]'>{item.title}</h3>
