...
export default function TitleType({ addClass, children, titleType }) {
    return (
        <>
            {
                titleType === 'h1' ?
                    <h1 className={`text-2xl lg:text-4xl font-secondary text-black ${addClass}`} >{children}</h1>
                    :
                    titleType === 'h2' ?
                        <h2 className={`text-center text-2xl lg:text-3xl font-secondary text-secondary font-bold mb-8 ${addClass}`} >{children}</h2>
                        :
                        titleType === 'h3' ?
